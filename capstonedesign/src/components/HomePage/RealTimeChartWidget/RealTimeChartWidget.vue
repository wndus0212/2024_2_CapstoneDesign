  ...
  methods: {
    ...
    fetchAllIndexData() {
      const requests = this.stockIndices.map(index => {
        // history 데이터 요청
        const historyRequest = axios.get(`https://port-0-capstonedesign-m3vkxnzga0885b97.sel4.cloudtype.app/stock/index/index/${index.value}/`, {
          params: {
        ...
        const diffRequest = axios.get(`https://port-0-capstonedesign-m3vkxnzga0885b97.sel4.cloudtype.app/stock/stock_diff/${index.value}/`) // 예시 URL
          .then(response => {
            if (!response.data) {
              console.log(response);
