  ...
  export default {
    ...
    data() {
      return {
        chartOptions: {
          ...
          title: {
            text: 'Final Cash Value Distribution',
            align: 'left',
          },
          xaxis: {
            title: {
              text: 'Final Cash Value',
            },
    ...
    methods: {
        updateHistogramData(data) {
            ...
            // 최종 현금값만 추출
            const finalCashValues = data
            // 히스토그램 구간을 만들기 위해 데이터의 최소값과 최대값을 구함
            const minValue = Math.min(...finalCashValues);
            const maxValue = Math.max(...finalCashValues);
