...
class FixedAllocationStrategy(bt.Strategy):
    params = (
        ...
        ('withdrawal_percentage', 0.05),  # 리밸런싱 시 인출 비율 (5%)
    )
    ...
    def __init__(self, allocation):
        ...
        self.total_withdrawn_cash = 0  # 누적 인출 금액
    ...
    def rebalance_portfolio(self):
        """
        목표 비중에 맞춰 포트폴리오를 리밸런싱하고 일정 비율의 자금을 인출합니다.
        """
        total_value = self.broker.get_value()
        positions = {}  # 현재 포지션 정보 저장

        # 자금 인출 계산
        withdrawal_amount = total_value * self.params.withdrawal_percentage
        self.total_withdrawn_cash += withdrawal_amount  # 누적 인출 금액 갱신
        self.broker.cash -= withdrawal_amount  # 인출
        ...
        for data in self.datas:
            ...
            target_value = (total_value - withdrawal_amount) * self.allocation.get(ticker, 0)
        ...
        self.rebalance_log.append({
            ...
            'positions': positions,
            'withdrawn_cash': withdrawal_amount,  # 현재 리밸런싱에서 인출된 금액
            'total_withdrawn_cash': self.total_withdrawn_cash,  # 누적 인출 금액
        })

        print(f"리밸런싱 완료: {self.last_rebalance}")
        print(f"포트폴리오 이름: {self.params.portfolio_name}")
        print(f"포지션: {positions}")
        print(f"이번 리밸런싱 인출 금액: {withdrawal_amount:,.2f}")
        print(f"누적 인출 금액: {self.total_withdrawn_cash:,.2f}")
...
def save_rebalance_log_to_csv(rebalance_log, output_file):
    ...
    with open(output_file, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["portfolio_name", "date", "ticker", "position", "withdrawn_cash", "total_withdrawn_cash"])  # 헤더 작성

        for entry in rebalance_log:
            portfolio_name = entry['portfolio_name']
            date = entry['date']
            withdrawn_cash = entry['withdrawn_cash']
            total_withdrawn_cash = entry['total_withdrawn_cash']
            for ticker, position in entry['positions'].items():
                writer.writerow([portfolio_name, date, ticker, position, withdrawn_cash, total_withdrawn_cash])
